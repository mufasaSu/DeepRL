...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + self.discount * (1 - done.type(torch.int64)) * next_qs

            if self.use_entropy_bonus and self.backup_entropy:
                # TODO(student): Add entropy bonus to the target values for SAC
                next_action_entropies = self.entropy(next_action_distribution)
                next_qs += self.temperature *  next_action_entropies #* self.discount # before: without self.discount; next_qs = error (1 - done.type(torch.int64)) *
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        entropy_val = torch.mean(entropy)
        # TODO: Use multiple samples!
        # entropy = torch.empty(size=(action_distribution.batch_shape[0],))
        # for _ in range(self.num_actor_samples):
        #     entropy += -action_distribution.log_prob(action_distribution.rsample())
        # entropy /= self.num_actor_samples
        return entropy_val
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            #q_values = self.critic(obs[None].expand((self.num_critic_networks, batch_size, -1)), action)
        ...
        return loss, self.entropy(action_distribution)#torch.mean(self.entropy(action_distribution))
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        if self.use_entropy_bonus:
            loss -= self.temperature * entropy #TODO: Before: -=
        # For Pendulum sanity:
        # action_distribution = self.actor(obs)

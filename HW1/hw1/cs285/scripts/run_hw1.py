#TODO: check action dimension and action[0],
# check why reward min max are the same 
"""
...
def run_training_loop(params):
    ...
    # replay buffer
    replay_buffer = ReplayBuffer(params['max_replay_buffer_size']) # store experiences
    ...
    # add logging into csv
    itr_logs = pd.DataFrame(columns=['Eval_AverageReturn', 'Eval_StdReturn', 'Train_AverageReturn', 'Train_StdReturn'], index=['Iteration'])
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths = pickle.load(open(params['expert_data'], 'rb')) # originally 2 paths
            ...
            if params['do_dagger']:
                ...
                for i in range(len(paths)):
                    paths[i]["action"] = expert_policy.get_action(paths[i]["observation"]) #TODO
        ...
        training_logs = []
        for _ in range(params['num_agent_train_steps_per_iter']): # wie viele batches sammeln
        ...
        if log_metrics:
            ...
            # TODO: Remove!!
            itr_logs.loc[itr] = [logs["Eval_AverageReturn"], logs["Eval_StdReturn"], logs["Train_AverageReturn"], logs["Train_StdReturn"]]
            itr_logs.to_csv(r'C:\Users\musta\Desktop\WS23\DeepRL\Homeworks\homework_fall2023\plots\itr_logs.csv')
...
def main():
    ...
    parser.add_argument('--expert_policy_file', '-epf', type=str, required=False, default=r".\policies\experts\Hopper.pkl")  # relative to where you're running this script from
    parser.add_argument('--expert_data', '-ed', type=str, required=False, default=r".\expert_data\expert_data_Hopper-v4.pkl") #relative to where you're running this script from
    parser.add_argument('--env_name', '-env', type=str, help=f'choices: {", ".join(MJ_ENV_NAMES)}', required=False, default="Hopper-v4")
    parser.add_argument('--exp_name', '-exp', type=str, required=False, default="dagger_hopper") #default='pick an experiment name', 
    parser.add_argument('--do_dagger', default=True)#action='store_true')
    parser.add_argument('--ep_len', type=int)

    parser.add_argument('--num_agent_train_steps_per_iter', type=int, default=1200)  # number of gradient steps for training policy (per iter in n_iter)

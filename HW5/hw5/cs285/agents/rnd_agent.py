...
class RNDAgent(DQNAgent):
    ...
    def update_rnd(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        #loss = torch.nn.MSELoss()(self.rnd_target_net(obs), self.rnd_net(obs)) 
        #loss = torch.sqrt((self.rnd_net(obs) - self.rnd_target_net(obs)).pow(2).sum(dim=1)).mean() 
        loss = torch.norm((self.rnd_net(obs) - self.rnd_target_net(obs)).pow(2), dim=-1).mean() # TODO double check here
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            #norm version:
            rnd_error = torch.norm(self.rnd_net(observations) - self.rnd_target_net(observations), dim=-1) #.pow(2)
            #rnd_error = (self.rnd_net(observations) - self.rnd_target_net(observations)).pow(2).mean(dim=1)
            #rnd_error = torch.sqrt((self.rnd_net(obs) - self.rnd_target_net(obs)).pow(2).sum(dim=1))
            #a = (self.rnd_net(observations) - self.rnd_target_net(observations)).pow(2).mean(dim=1).mean(dim=0)
            #b = torch.nn.MSELoss()(self.rnd_target_net(observations), self.rnd_net(observations))
            # a and b are the same!
            assert rnd_error.shape == rewards.shape
            #rewards = rewards.float() + self.rnd_weight * (rnd_error - rnd_error.mean()) / (rnd_error.std() + 1e-8)

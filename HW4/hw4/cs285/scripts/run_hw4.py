...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_ob_ensemble = np.array([mb_agent.get_dynamics_predictions(i, ob[None, :], ac[None, :]) for i in range(mb_agent.ensemble_size)])
        next_ob = np.mean(next_ob_ensemble, axis=0)[0]
        obs.append(ob)
        acs.append(ac)
        rewards.append(env.get_reward(ob, ac)[0])
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                ...
                step_losses += [mb_agent.update(i, batch["observations"], batch["actions"], batch["next_observations"])]
        ...
        # if itr == 0:
        #     #print("skkkrr")
        #     #print(os.path.join(logger._log_dir, "itr_0_loss_curve.png"))
        #     plt.plot(all_losses)
        ...
        #     # Given string
        #     s = args.config_file
        #     # Regular expression to extract the desired substring
        #     match = re.search(r'([^/]+)\.yaml$', s)
        #     if match:
        #         result = match.group(1)
        #     plt.savefig(r"C:\Users\musta\Desktop\WS23\DeepRL\Homeworks_forked\homework_fall2023\hw4\plots\\" + result + ".png")
            #print("kkrrr")
...
def main():
    ...
    parser.add_argument("--config_file", "-cfg", type=str, default="experiments/mpc/halfcheetah_mbpo.yaml")#required=True,
    parser.add_argument("--sac_config_file", type=str, default="experiments/sac/halfcheetah_clipq_rl1.yaml")

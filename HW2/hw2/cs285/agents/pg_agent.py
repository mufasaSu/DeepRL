...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            #values = self.critic.forward(ptu.from_numpy(obs)).squeeze().detach().numpy()
            with torch.no_grad():
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        gamma_discount = np.array([self.gamma ** i for i in range(len(rewards))])
        # multiply each reward in rewards by the gamma_discount array
        disc_rewards = np.sum(gamma_discount * np.array(rewards)) * np.ones(len(rewards))
        return disc_rewards
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        gamma_discount = np.array([self.gamma ** i for i in range(len(rewards))])
        disc_rewards_to_go = [np.sum(gamma_discount[i:] * np.array(rewards[i:])) for i in range(len(rewards))]

        return disc_rewards_to_go
